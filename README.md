...
All skills require explicit `$codex-cc-triage:<skill>` invocation by the user or an
already-authorized owning workflow; they do not start unsolicited paid opinions. The question, plan, and review
...
worktree mutation and fails the call. The wrapper verifies Python and authentication before each dispatch. Successful CLI capability
...
Wrong required bases fail before model dispatch, and any new reply/failure revokes old approval. The lifecycle pins base,
