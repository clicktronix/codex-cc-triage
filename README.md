...
Claude CLI flags, and `claude auth status`; incompatible or unauthenticated installations fail
before repository content is sent.
...
Each Claude dispatch also has a portable 15-minute wall-clock timeout by default. Timeout and CLI
failures preserve stderr and any partial JSON under the named thread's diagnostics.
...
| `CODEX_CC_TRIAGE_TIMEOUT_SECONDS` | `900` | Per-call wall-clock timeout |
